...
| 5. 렌더된 결과 재생 | ✅ 5-1·5-1b·5-2 모두 검증 완료 | `4b952b6` + 후속 |
...
커버로 쓸지는 여전히 제품 판단이며, 쓰기로 하면 필요한 일:

- `MovieRender`에 썸네일 필드 추가 + `finishMovieJob`에서 저장 (URL은 만료되므로 `render.videoId`로 재조회하는 `useRenderSource`와 같은 방식이 필요 — 저장된 URL을 커버로 쓰면 며칠 뒤 깨진 이미지가 됩니다)
- `MovieSummary.coverUris`가 완성된 무비에서는 서버 썸네일을 우선하도록
- 스냅 원본이 지워진 무비도 커버를 갖게 되는 이득이 여기서 생깁니다(지금은 커버가 빈칸)

안 쓸 거면 `get-edited-video.ts`에서 필드를 지우세요 — 지금은 읽히지 않는 데이터입니다.
