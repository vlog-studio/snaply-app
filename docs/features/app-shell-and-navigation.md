...
Snaply opens into a two-tab application with Home (오늘) and Archive (보관함) destinations, and a floating amber safelight button centered over the tab bar between them (moment-collection redesign, concept §6). The tab bar shows icons only (no text labels); the Home and Archive labels are retained for accessibility and route-name fallback but hidden. Capture is an action rather than a tab: the center safelight button opens `/capture` from either tab, which the root stack presents as a full-screen modal over the tab navigator. `/capture` is the darkroom viewfinder itself (mood/duration are tuned inline, not on a separate setup screen); the three capture screens (`/capture` plus `/capture/editing` and `/capture/result`) are presented by the root stack outside the tab navigator. Settings is no longer a tab; it is reached from a corner control on the Archive screen and pushed as a normal root-stack screen.
...
| Screen-owned bottom chrome takeover | Removed | A screen used to be able to hide the tab bar and safelight so its own action bar could take their place (`shared/ui/tab-bar-chrome`). Its only user was the Archive clip selection mode, which now lives on the pushed `/cuts` route where no tab bar exists, so both the switch and the hiding were removed. |
...
| Modal Capture entry | `Functional` | The center safelight button opens `/capture` — the darkroom viewfinder — as a root-stack full-screen modal (`presentation: 'fullScreenModal'`, header hidden); a close button on the screen returns to the previous tab. |
| Native animated splash transition | `Functional` | The system splash is hidden after the overlay lays out, then the overlay — the Snaply moment-ring mark (`assets/images/brand-glyph-ember.png`, 150dp) on the darkroom ground background — fades out. The overlay's resting frame mirrors the native `expo-splash-screen` config exactly, so the handoff is seamless. |
| Web splash transition | `Functional` | The overlay intentionally renders nothing on web. |
| Dark-fixed darkroom theme | `Functional` | The app is a single dark world — the "darkroom" (moment-collection redesign, concept §5). `useTheme`/`useResolvedColorScheme` always resolve to the dark darkroom palette regardless of OS appearance; there is no light theme, no in-app theme toggle, and no stored theme preference. |
...
| `/capture` | Headerless root-stack full-screen modal (opened by the center safelight button); the darkroom viewfinder | `pages/capture-record` |
...
| `/cuts` | Root-stack screen with a themed native header ("컷"), opened from the Archive's cut drawer | `pages/cut-strip` |
...
- `src/_app/providers/app-providers.tsx` fixes the Expo Router navigation theme, status-bar style, and Android navigation-bar button style to the dark darkroom palette. It also mounts three headless nodes for the whole authenticated session — `PushTokenRegistrar`, `GeofenceGate`, and `DailyRollGate` (ensures today's roll exists after the roll store hydrates) — which own no UI; see [Location alerts and push notifications](location-and-push-notifications.md) and [Roll detail](roll-detail.md). It wraps its children in `GestureHandlerRootView`, the ancestor `react-native-gesture-handler` gestures require (used by the roll-detail drag reorder), which expo-router's native stack does not provide.
...
- The theme is dark-fixed; the OS appearance setting has no effect. The persisted theme-mode store that predated the darkroom conversion has been removed — nothing read it, and `useTheme` documented that it was ignored.
