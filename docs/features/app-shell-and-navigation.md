...
| Screen-owned bottom chrome takeover | `Functional` | A screen can temporarily replace the bottom chrome with its own action bar: `shared/ui/tab-bar-chrome` holds a `hidden` switch that `app-tabs.tsx` reads to hide both the tab bar (`display: 'none'`) and the safelight overlay. Currently used by the Archive clip selection mode, which shows a bottom delete bar in the tab bar's place; the owning screen is responsible for restoring the chrome on exit/blur. |
...
- `src/shared/ui/tab-bar-chrome` owns the bottom-chrome `hidden` switch shared between the tab navigator and screens that temporarily replace the tab bar with their own action bar (see [Recording archive](recording-archive.md)).
