...
- `src/features/watch-reward-ad` — the three `/billing/ad-rewards` calls, `adRewardQueries.availability()`, the `RewardAdProvider` seam (the AdMob implementation will replace `mockRewardAdProvider` in one place, `use-watch-reward-ad.ts`), and the `useWatchRewardAd` state machine (`idle → preparing → showing → settling`).
