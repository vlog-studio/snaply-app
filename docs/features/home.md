...
| Contact-sheet film strip | `Partial` | The strip shows one undeveloped negative frame per real clip in today's roll, followed by dashed empty slots up to the `/12` target. It scrolls horizontally, so every clip is reachable no matter how many the roll holds — there is no 6-frame preview cap. Each filled frame is a frosted negative: the clip's first frame is sampled (`shared/ui/negative-frame` via the disk-cached `shared/lib/video-thumbnails` util) and rendered heavily blurred, dimmed, and amber-washed with its edge index on top — present and unmistakably filled, but the moment stays illegible until develop (미현상). Tapping the strip opens the full roll at `/roll/[id]`. The perforation styling is decorative. |
...
- `src/entities/clip` resolves today's roll references into the ordered clips the strip draws (`useClipsByRefs`); `src/shared/lib/datetime` formats the develop-notice reel estimate (`formatDuration`). `src/widgets/roll-shelf` is not used by Home — its roll read models feed only the archive cabinet.
- The edge print's date comes from today's roll itself (`rollDate`), not from the clock, so it names the roll on screen and cannot disagree with it when the app is left open past midnight. It is blank until `DailyRollGate` establishes the roll, exactly as the roll ordinal already was.
...
- The contact-sheet frame count is derived from today's roll. Filled frames now show a frosted, heavily blurred + amber-washed sample of each clip's first frame (`shared/ui/negative-frame`) so a captured frame reads as present and real, while the moment stays unrecognizable (undeveloped) by design. Frames are extracted through the disk-cached, one-shot `shared/lib/video-thumbnails` util (each clip is sampled at most once and the result is shared with the cut grid and roll detail), so the horizontally scrolling strip can show the whole roll without capping — it no longer holds a live video player per frame, which previously exhausted the platform's hardware decoders and left all but the last frame black.
...
- "촬영 중 테마 롤" from the concept §6 Home description is not shown — themed rolls are deferred to v1.1+. Free rolls bundled by hand (see [Recording archive](recording-archive.md#two-kinds-of-roll)) never appear on Home either: Home is today's daily roll only, and the edge print's roll ordinal counts daily rolls alone, so bundling rolls in the archive does not renumber it.
