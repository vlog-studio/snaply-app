...
| Toggle movie-completion alerts (무비 완성 알림) | `Partial` | Persists across restarts (SecureStore) and gates a real notification: with it on, a generation job that ends while the app is running presents a local notification ("무비가 완성됐어요" / "무비를 만들지 못했어요"). Turning it on requests the OS notification permission; a refusal leaves the switch off and the row says so. Native only. Nothing arrives when the app has been force-quit, because the job runs on the device — see [The movie screen](movie.md). |
