...
The screen does not say so. Two earlier attempts at saying it were both removed for their own reasons — an `AI가 고른 이유` panel ran four lines deep and pushed the slots themselves, the one thing the screen exists to show, under the fold; a per-row `같은 외출 확신 NN%` caption repeated a constant on every row (below). What is left is a bare `NN%` and a layout that keeps it away from the slot label. **No words on the screen state the limit or the basis**; that gap is the screen's one open issue, recorded under [Known limitations](#known-limitations). Neither removed form is the fix to reach for.
...
The number carries no caption. `같은 외출 확신 NN%` was tried and dropped: six rows of one contiguous outing all score the same, so the caption repeated a constant six times and cost a third of each row's width. What carries the meaning instead is layout: the percentage stays pushed to the far edge of the row, away from the label, because `골목 70%` set flush together reads as "70% sure this is an alley" — the one claim the match cannot make. **Nothing on the screen now says what the number measures.** See [Known limitations](#known-limitations).
...
- **Nothing on the screen says the app has not looked at the pictures, and nothing says what `NN%` measures.** The `AI가 고른 이유` panel and the `같은 외출 확신` caption were both removed, and between them they took every word that explained the match. A first-time user sees `골목`, a photo, and `70%`, whose most natural reading — "70% sure this is an alley" — is exactly the claim [What the match can and cannot do](#what-the-match-can-and-cannot-do) says the build must never make. Layout still separates the number from the label, which is not the same as saying so. **This is the one open issue on the screen**: it costs a single muted line under the header, and it should go back before the screen is shown to anyone who has not been told how the match works.
