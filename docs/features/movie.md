...
**A finished movie plays its cuts back to back**, each inside its trim window, unless a run produced a file. The backend does render one now (`render.uri`), but the player that shows it is still the cut player — playing a single remote file is [step 5 of the integration](#known-limitations) and is not done yet. Until then what the user watches is the order and lengths they settled on, which is also what previews a `draft` so a trim can be judged before the run is started.
...
- **The backend composites, but nothing plays what it made.** A run really does render a file and `render.uri` holds it (2026-08-07), yet the stage still plays the cut list — so what the user watches is their own originals in order, not the edit that was produced. Style is real now (it picks the preset that renders); BGM and 자동 자막 are not, because the pipeline chooses the track from the preset and subtitles every run regardless. **The screen says none of this** — the footer's `아직 프로토타입` line went with the summary prose on 2026-08-05. Restore a disclosure before this reaches anyone outside the team.
- **The result is 1920×1080 landscape.** The pipeline pads every clip into a 16:9 canvas, so a portrait snap comes back pillarboxed — while the app is 9:16 only and calls itself a short-form vlog studio. A vertical render is being done on the backend side; until it lands, every file `render.uri` points at is the wrong shape for the product.
- **Trim, BGM, title, and ratio do not travel.** `POST /edit-jobs` takes the cut ids and a style preset and nothing else, so a trimmed cut renders in full and the movie's own track and name never reach the renderer. Trim support is planned on the backend; the studio keeps its trim UI and states that this run will not reflect it.
