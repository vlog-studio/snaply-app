...
| `generating` | The progress ring and the backend's current stage fill the stage; the strip stays visible as a read-out (no add tile, no inspector), the sheets open read-only, and the footer is empty. Leaving is expected. |
...
| --- | --- | --- |
| AI로 생성 시작 | `Functional` | Queues a real run on the backend (`POST /edit-jobs`, the cuts in cut order — each with its trim window in milliseconds — plus the style preset and the output shape `short_vertical`/`blur_background`) and puts the movie into `generating` **only once there is a `jobId` to follow** — the socket and the status endpoint are both addressed by that id, so a movie that went `generating` before the request landed would be a job nothing could report on. The backend renders the cuts, matches music to the preset, transcribes subtitles, and uploads the result; the movie becomes `ready` with a `render` holding the file's URL, its length, and the cut list it was made from (`render.snapRefs`, frozen by `finishMovieJob` — see [Composing and fixing it](#composing-and-fixing-it)). In mock mode nothing is composited and the movie finishes without a file, exactly as every movie did before 2026-08-07. |
| Progress | `Functional` | A ring filled to the percentage the pipeline last published, and under it the stage it named, in the backend's own words (`컷편집 완료`, `음악 매칭 중...`). Both are stored on the movie (`MovieJob.progress` / `.step`), so every surface drawing it agrees and none of them ticks — the job clock and the five-step checklist went with the local simulation on 2026-08-07. Milestones are coarse (six over a run), so the ring holds still for stretches; nothing interpolates, because nothing here knows how far into a stage the render is. A card elsewhere shows the same fraction as a bar (see [Studio and movies](studio.md)). |
| Leaving mid-job | `Functional` | The run belongs to the backend and the job id belongs to the movie, so neither depends on the screen: `MovieGenerationGate` is mounted app-wide by `_app`'s `MovieGenerationBridge`, and a run keeps going while the user browses other tabs, backgrounds the app, or force-quits it. `useGenerationRunner` hears about it three ways — the progress socket while the app is foregrounded, a poll every 20 seconds under it, and a catch-up pass on every return to the foreground and on app start. A run that finished while the app was away is therefore picked up the moment it comes back. |
...
| Announce the end | `Partial` | With 무비 완성 알림 on ([Me tab](me.md)), a run that ends — either way — presents a local notification, so the user who walked away is told. It is raised by the app when it *learns* the run ended, not by the server when the run ends, so a force-quit app announces nothing until it is opened again (and then announces immediately, on the catch-up pass). A push from the backend would close that gap; the notification API for it does not exist yet ([Location and push](location-and-push-notifications.md)). |
...
- **The backend says so.** A pipeline error, or a render that ran past its timeout. The stored reason is the server's message, shown as-is in the footer notice above `다시 시도`.
- **The material went.** The user deletes the last original a running job was built from. Deleting a snap strips it from every movie that references it ([Snap library](snaps.md#deleting-an-original)), and a job with no cuts left can only produce nothing — so the app fails it locally rather than waiting for a render of an empty list.
...
| Rendered file | `Partial` | `Movie.render.uri` now holds the backend's rendered file when a run produced one (mock mode produces none). **Nothing plays it yet** — the player still runs the cut list — so a movie's stage shows its own originals rather than the edit the backend made of them. Playing a single remote file is the next step; this player stays for movies generated without one. |
...
| Capability | Status | Actual behavior |
| --- | --- | --- |
| 무비 공유 | `Not implemented` | **Worse than before, and now reachable (2026-08-10):** real runs fill `Movie.render.uri`, which *enables* the 공유 button — but `useShareMovie` hands that `https://` URL straight to `expo-sharing`, which accepts local files only, so the share fails with nothing but a dev-log warning. Step 5-2 of the integration (download the file to cache first, then share the local path) is what fixes this; until it lands the button is a live control that does nothing. Three surfaces offer it: watch mode's 공유 button (and the ⋯ sheet's 공유하기 row), the studio footer's control, and the movie tab's selection bar while one movie with a file is selected ([Studio and movies](studio.md)). |
...
- **Playback of the finished file is blocked on a backend fix (2026-08-10).** The worker stores an *unsigned* object URL in `videos.edited_url` and the API returns it verbatim, but the bucket is private — so every address the app gets, stored or freshly fetched, answers AccessDenied. The app side is ready (`useRenderSource` re-asks by `render.videoId` on every visit and plays whatever the backend hands out); what is missing is the backend presigning a GET URL at `GET /videos/{id}` response time. Until that lands, watch mode's file stage reaches its error face on a real server.
- **The studio's preview is the raw cuts, not the coming render.** Watch mode plays the finished file now (2026-08-10), but the studio stage still previews the editable cut list — by design (it is what a change is judged against) — and that preview carries none of what the run will add: no music, no subtitles, no style grading, no `fitMode` background. Nothing on the studio face says the result will differ from the preview. A disclosure is still owed before this reaches anyone outside the team.
- **The output shape is asked for but not yet seen.** `POST /edit-jobs` takes an output profile as of 2026-08-10 and the app asks for `short_vertical` (1080×1920) with `blur_background` fit, rather than leaning on the server's identical defaults. The backend's own profile table defines that size and its worker renders to it, so the 16:9 pillarboxing that every earlier run produced should be gone — but **no run has been watched on a real server since**, so treat the shape as unverified. `blur_background` puts a blurred copy of the clip beside a landscape snap; whether the product wants black bars (`contain`) instead is undecided, and the value is a constant in `create-edit-job.ts` when it is.
- **Trim travels, but the renderer may still ignore it.** The request carries each cut's window (`clips: [{videoId, startMs, endMs}]`) as of 2026-08-10, so the app no longer withholds what the user trimmed. The backend's worker, however, reads only the style preset out of the job's edit spec in the state this was written against — the API accepts the windows and stores them, and applying them is backend work that may not have landed. Until a real run is watched, a trimmed cut may still come back whole; the studio's trim UI stands either way.
- **BGM and title do not travel.** The pipeline picks a track from the style preset and takes no track id, and the movie's name never reaches the renderer.
