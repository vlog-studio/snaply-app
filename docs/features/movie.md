...
| `ready` | The stage plays the result, the same controls, and the footer offers `이 구성으로 다시 만들기` beside 공유. If the cut list has been edited since the render, a notice above the footer says what plays is the changed composition and offers `완성 당시 구성으로 되돌리기`. |
...
| Capability | Status | Actual behavior |
| --- | --- | --- |
| 무비 공유 | `Not implemented` | The export path is complete — `features/share-movie`'s `useShareMovie` hands `Movie.render.uri` to the OS share sheet through `shared/lib/sharing` (`expo-sharing`) — but no movie has a rendered file. Two surfaces offer it, each in its own idiom: this screen's footer keeps the control visible but disabled, while the movie tab's actions sheet shows its 공유 row only when a rendered file exists — so today it shows none ([Studio and movies](studio.md)). |
...
- `src/features/rename-movie` owns the rename form (`RenameMovieForm`), the sheet that wraps it (`RenameMovieSheet`), and its schema. A feature rather than page code because two surfaces need it: this screen opens the sheet, and the movie tab's actions sheet draws the bare form inside its own modal.
- `src/features/share-movie` owns the export decision (`useShareMovie`) — what may be handed to the share sheet and why a movie is blocked. A feature because this screen's footer and the movie tab's actions sheet offer the same act.
