...
- **Trim travels, but its rendering is unverified.** The request carries each cut's window (`clips: [{videoId, startMs, endMs}]`) as of 2026-08-10, so the app no longer withholds what the user trimmed. The backend's worker, however, read only the style preset out of the job's edit spec in the state this was written against — the API accepts the windows and stores them, and applying them is backend work that may not have landed. No run with a trimmed cut has been watched on a real server yet, so a trimmed cut may still come back whole; the studio's trim UI stands either way.
- **`fitMode` is undecided.** The app sends `blur_background`, which puts a blurred copy of the clip beside a landscape snap; whether the product wants black bars (`contain`) instead is an open call, and the value is a constant in `create-edit-job.ts` when it is made.
