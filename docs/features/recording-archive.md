...
└── 선택 모드        long-press or 선택 → 롤에 담기 · (롤별 필터에서) 롤에서 빼기 · 삭제
```
...
- `src/pages/cut-strip` owns the `/cuts` screen. `model/use-cut-strip` is its read model: it joins `entities/clip` with `widgets/clip-membership`, applies the filter, groups cuts into days, and derives each day's status and today's open frames (`useCutRollFilters` supplies the roll picker's options). `ui/cut-film-strip` owns one day's strip and its perforation, `ui/cut-frame` a single frame with its roll dots, `ui/cut-filter-bar` the filter chips, `ui/cut-roll-picker-sheet` the roll picker that narrows the strip, `ui/cut-add-to-roll-sheet` the picker that chooses where cuts go, `ui/cut-sheet` the per-cut sheet, `ui/cut-delete-dialog` the delete confirmation, `ui/cut-selection-bar` the contextual bottom action bar, and `ui/roll-status-label` the one Korean status wording the sheet and the dialog share.
...
- `src/features/collect-clips` owns membership changes — `롤에 담기` and `롤에서 빼기` (`useCollectClips`) plus the rolls a selection may be put into (`useCollectTargets`, undeveloped rolls only, today's first, with how many of the selection each already holds). Both the contact strip and roll detail change membership, which is what makes it a feature; concentrating it also concentrates the frozen-membership rule, which it enforces by reading the roll at call time instead of trusting a disabled control. It reports what it actually did (`CollectOutcome`), so a refusal is visible rather than silent. 새 롤로 묶기 and retiring an emptied manual roll belong here too and arrive with manual roll creation.
...
- 새 롤로 묶기 is not built, so cuts can only be put into rolls that already exist — in practice the daily rolls. Until a roll can be made by hand, the selection bar leads with 롤에 담기 rather than the design's primary action, and a cut in no roll can only be collected into a day's roll.
- Collecting gives no confirmation beyond the frame's dots changing; the design's fly-to-cover motion is not built.
