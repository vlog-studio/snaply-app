...
- `src/widgets/roll-shelf` owns the roll read models — `useRollsAwaitingDevelop` (today + everything still unfinished) and `useDevelopedRollMonths` (developed rolls by month with each month's empty-day count). Both summarize a roll into cover URIs, counts, length, tint, the day it is filed under (`date`), and the span its cuts cover (`dayRange`), resolving its references through `entities/clip`'s `clipsByRefs` over a single `useClipIndex` pass. `clipCount` is counted from the references rather than the resolved clips, so a roll whose original was deleted still reports the frame it occupied.
...
- `src/features/manage-recordings` owns listing and saving recording files. Korean date/time/day and duration formatting is `shared/lib/datetime` — a formatter is business-agnostic, so a page prints one without depending on this feature. It owns no deletion path — that moved to `features/delete-clip` so no caller can delete a file without the cascade.
