...
- `src/pages/roll-detail` owns the screen composition, the edit modes (selection / reorder state machine), and the roll↔clip joins (`model/use-roll-detail.ts` for the roll's cuts, `model/use-addable-clips.ts` for the archive clips still addable to it). Its `ui` segment owns `cut-sheet-grid` (the contact sheet for every mode: absolutely-positioned cells so a cut can animate between slots, per-cut Pressable for view/select, gesture-handler pan armed only in reorder mode, reanimated springs for the step-aside/settle motion; one grid so the cells — and their thumbnails — never remount on a mode change), `clip-player-modal` (the roll's wording for the shared player), and `add-clips-sheet` (clip picker). `model/reorder-layout.ts` holds the pure slot geometry and order-shuffle math the drag worklets evaluate (unit-tested; worklet-marked). The gestures rely on the `GestureHandlerRootView` mounted by `src/_app/providers`.
