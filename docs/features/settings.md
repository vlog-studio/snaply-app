...
The app is now dark-fixed (the "darkroom", per the moment-collection redesign), so Settings no longer exposes a screen-theme control, and the theme-mode store that once backed one has been removed — `useTheme`/`useResolvedColorScheme` always resolve to the dark darkroom palette. The page otherwise imports only shared theme and typography modules.
