...
`Partial` — the flow is implemented end-to-end (picker → window → native trim → snap → background upload) and unit-tested at every model seam, but it ships a **new native module** that has not yet been verified on hardware: Android on-device verification is pending, and iOS has not been built at all (the development Mac's Xcode cannot build SDK 57 natively). Web is excluded by design — videos never persist there.
...
| Extraction | `Functional`* | `features/extract-snap`: native trim (`shared/lib/video-trim`) → `persistLocalRecording` → snap metadata → `addSnap`. From there the snap is indistinguishable from a captured one — the upload worker finds it pending, the library lists it. Success gives haptics, a `담김 · 스냅 N개` badge, and a counter in the top pill; the screen stays for the next cut. *Functional in JS terms; the native layer is unverified on hardware (see status summary). |
