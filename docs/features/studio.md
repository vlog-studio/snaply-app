...
| Movie actions: rename / share / delete | `Functional` | A long press on a movie-tab tile — the same gesture that starts a snap delete — opens an actions sheet (`pages/movies/ui/movie-actions-sheet.tsx`): a centered plain header naming the movie, its cut count, length, and date, then the actions in a single bordered group split by hairlines, iOS-action-sheet style. Rename and delete are steps *inside* the one sheet rather than sheets of their own, because closing one RN Modal while presenting the next is a race iOS regularly loses; cancelling a step returns to the menu. **이름 바꾸기** draws `features/rename-movie`'s form in place — the same rename the movie screen offers. **공유** goes through `features/share-movie` and appears only while there is a rendered file to hand over — which today is never (see [The movie screen](movie.md#sharing)), so the menu holds two rows until a renderer fills `Movie.render.uri`. **삭제** steps to a confirmation (`ui/movie-delete-confirm.tsx`); confirming calls `useDeleteMovie`: one synchronous store write, because a movie is only a composition — the snap originals, their thumbnails, and the tray are untouched, and the step says so instead of warning in the abstract. A `generating` movie can be deleted too; the step warns that the job in flight goes with it (the runner simply stops finding the movie on its next tick). The grid is the only entry: board rows offer no long-press actions, and the movie screen deletes nothing. |
...
- `src/pages/movies` owns the movie tab's grid and the long-press actions sheet (`ui/movie-actions-sheet.tsx`, with its delete step in `ui/movie-delete-confirm.tsx`) — page-local because the grid is the actions' only entry point. Rename and share are not its own: the sheet composes `features/rename-movie`'s form and `features/share-movie`'s export decision.
...
- The long-press actions are reachable from the movie tab alone: a board row answers a long press with nothing, so a movie made by mistake still has to be found in the grid to be removed. (Rename alone has a second home, on the movie screen.)
