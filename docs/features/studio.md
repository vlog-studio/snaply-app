...
## Replaced: the 담기 트레이 (removed 2026-08-12)

The tray — a persistent basket that picks landed in before becoming a movie — was the concept's one invention (concept §5), there so material could be gathered across several days. It is gone because the **draft movie already does everything it did, without the extra stop**: a draft persists across restarts, takes more snaps later through the movie screen's 스냅 더 넣기, obeys the same ten-snap cap, and loses deleted originals through the same cascade — and drafts are plural, so gathering for two movies at once (the tray's documented limitation) simply works. Confirming a selection on the Snap tab now creates a `user`-arranged draft directly and opens it; the 담기 → 스튜디오 → 이 스냅으로 새 무비 detour, the 담김 badge, the 트레이 chip, and the `나` tab's tray pill left with it.

What remains on the studio is the entry: a one-row `새 무비 · 스냅 고르기` block (the weight the empty tray panel had already settled on) opening the Snap tab in selection mode. A leftover `snaply.tray` from an older build is promoted to a draft once at startup (`_app/providers/tray-draft-migration.tsx`) and the key is deleted.
