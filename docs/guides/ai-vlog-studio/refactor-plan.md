...
| 2. 편집기 ①조립 + 무비 생성 + 스튜디오 보드 | ⬜ 미착수 | — |
| 3. 편집기 ②스타일 ③생성 + 무비 재생·상세 | ⬜ 미착수 | — |
...
| 3 | 스타일·BGM을 고르고 생성을 돌리고 완성 무비를 재생한다 | 실패 복구·재생성이 없다 |
...
- 트림 UI의 형태(슬라이더 vs 초 단위 스테퍼). 단계 2에서 판단.
- 목표 길이(`targetSec`)를 사용자가 정하게 할지, 컷 합계를 그대로 쓸지.
- 무비 삭제 UI를 어디에 둘지(무비 탭 롱프레스 vs 편집기 안).
