...
**뒤로 미룸(v1.1+)**: 테마 롤·프리셋 카드(롤 성격 선택), 지향 화면비·가로 유도, "다르게 현상" 고급 편집, 단일 MP4 렌더링·공유 내보내기.

### 범위 변경 — 수동 롤 생성 편입 (결정: 2026-07-27)

보관함 리디자인(A+B 하이브리드) 확정과 함께 **수동 롤 생성과 같은 컷 여러 롤 재사용 UI를 MVP 범위로 옮겼습니다.** 원래 둘 다 v1.1로 미뤄져 있었습니다.

- **근거**: 컷과 롤을 N:M으로 나눈 이유가 "한 컷이 여러 롤에 산다"인데, 롤을 만들 수 없으면 데일리 롤 하나뿐이라 N:M이 실제로 발생하지 않습니다. 데이터 모델만 N:M이고 경험은 1:N인 상태가 됩니다.
- **범위에 들어온 것**: `자유(free)` 롤 생성(선택한 컷들을 묶기), 기존 미현상 롤에 담기, 롤에서 빼기, 컷별 소속 롤 표시.
- **여전히 v1.1**: 롤 **성격** 선택(프리셋 카드), 기간·위치·태그 수집 규칙, 지향 화면비 UI. 동작이 구현되지 않은 값을 컨트롤로 노출하지 않습니다.
- 화면·규칙·슬라이스 배치와 마일스톤(M8)은 [`archive-ab-hybrid.html`](archive-ab-hybrid.html) §3·§6·§7에 있습니다.
...
| **M8** | 보관함 리디자인 (A+B 하이브리드) | 필름 캐비닛(`/archive`) + 컨택트 스트립(`/cuts`) + 수동 롤 생성·연쇄 삭제·롤 틴트. 단계(M8-1~6)는 [`archive-ab-hybrid.html`](archive-ab-hybrid.html) §7 | 단계별 유닛 테스트 + 실기기 확인 |

M1→M2→M3가 핵심 루프 뼈대, M4가 "매직", M5·M6은 완성도, M7은 화면 설계(concept §6) 정합. M8은 보관함을 수집욕(concept §4)의 화면으로 개편하고, 수동 롤 생성으로 N:M을 데이터 모델이 아니라 실제 경험으로 만든다.
...
### M8 상세 — 보관함 리디자인 (추가: 2026-07-27, 구현 완료: 2026-07-27)

**배경**: 기존 보관함은 컷/롤 세그먼트 격자였고, concept §4 수집욕·§5 시각 언어와의 간극을 [`archive-redesign.html`](archive-redesign.html)에서 4안으로 진단·비교해 **A+B 하이브리드**로 확정했다 — 보관함은 **필름 캐비닛**(`/archive`, 현상 대기 레인 → 월별 선반 → 컷 서랍), 원본 컷은 **컨택트 스트립**(`/cuts`, 하루가 스트립 한 줄, 색 점이 롤 소속). 확정 설계·화면·상태·모션·FSD 배치·단계별 산출물은 [`archive-ab-hybrid.html`](archive-ab-hybrid.html)가 소유한다.

**결과**: M8-1(연쇄 삭제·롤 틴트·역인덱스) → M8-2(캐비닛 + `/cuts` 라우트 분리) → M8-3(격자→스트립, 컷 목록을 파일에서 `entities/clip`으로) → M8-4(담기·빼기 + 영향을 말하는 삭제 다이얼로그) → M8-5(새 롤로 묶기 — §1 범위 변경의 구현) → M8-6(문서 정합) 순으로 완료. 각 단계는 유닛 테스트와 안드로이드 실기기(무선 adb)에서 확인했고, iOS 실기기는 기기가 없어 미검증.

**설계에서 벗어난 결정 둘**(상세는 `archive-ab-hybrid.html` §6·§7에 함께 기록):

- 빈 자유 롤 정리를 `removeRoll` 공개 API 대신 **`entities/roll` 스토어 불변식**으로 — 컷 빼기와 원본 삭제 양쪽이 롤을 비울 수 있어, 호출자가 지키는 규칙으로 두면 결국 한쪽에서만 지켜진다.
- fly-to-cover 모션 **미도입** — `/cuts`에는 프레임이 날아갈 커버가 화면에 없다. 대신 새 롤 틴트의 확인 줄(250ms ease-out-back 스냅 + medium 햅틱)로 §7 릴리즈 스냅의 감각만 살렸다.
