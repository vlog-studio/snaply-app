  ...
  return (
    <ScrollView
      ...
      <Animated.View entering={FadeInDown.delay(100).duration(420)} style={[styles.preview, { backgroundColor: '#C4875B' }]}>
        ...
        <View style={styles.previewTopRow}>
          <View style={styles.previewBadge}>
            <ThemedText selectable={false} style={styles.previewBadgeText}>✦ AI EDIT</ThemedText>
          </View>
        ...
        <View style={styles.previewBottom}>
          <ThemedText type="heading" style={styles.whiteText}>카페의 한 장면</ThemedText>
        ...
        <View style={styles.tagRow}>
          {['✦ 반짝이', '♬ 효과음', '◐ 웜 필터'].map((item) => (
            <View key={item} style={[styles.editTag, { backgroundColor: theme.aiSoft }]}>
              <ThemedText selectable={false} type="smallBold" themeColor="ai">{item}</ThemedText>
            </View>
          ))}
        </View>
      </View>
      ...
      <View style={styles.actions}>
        ...
        <SnaplyButton title="다른 순간 촬영" variant="secondary" onPress={() => router.replace('/capture')} />
      </View>
    </ScrollView>
  );
}
...
const styles = StyleSheet.create({
  ...
  previewGlow: { position: 'absolute', width: 210, height: 210, borderRadius: 105, top: -90, right: -80, opacity: 0.5 },
  previewTopRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  previewBadge: { backgroundColor: 'rgba(18,23,46,0.55)', paddingHorizontal: Spacing.two, paddingVertical: Spacing.one, borderRadius: Radius.pill },
  ...
  editTag: { borderRadius: Radius.pill, paddingHorizontal: Spacing.three, paddingVertical: Spacing.two },
