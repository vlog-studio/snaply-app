...
export type StyleStepProps = {
  ...
  totalSec: number;
  /** False once a job owns the movie; the settings become a read-out. */
  canEdit: boolean;
  onChange: (patch: MovieStylePatch) => void;
};

/**
 * Step ② — the look and the sound (concept §6).
 *
 * Every control writes straight through: a style is one tap and a switch is one
 * flip, so there is nothing to stage and no save button to explain. The assemble
 * step buffers its edits for a different reason — its rule about a minimum cut
 * count has to be expressible as a disabled control.
 *
 * Ratio and target length are read-outs, not controls. 9:16 is the only ratio the
 * product has, and the target length follows the cuts: the length is decided by
 * trimming, which is a decision the user already made one step earlier.
 */
export function StyleStep({ movie, totalSec, canEdit, onChange }: StyleStepProps) {
